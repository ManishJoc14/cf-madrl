...
def main():
    ...
    if esp32_ips:
        ...
        if len(esp32_ips) == 1:
            ...
            if response == 'y':
                config_file = Path("yolo_inference.py")
                if config_file.exists():
                    content = config_file.read_text()
                    # Update ESP32_STREAM_URL
                    old_url = 'ESP32_STREAM_URL = "http://192.168.1.100:81/stream"'
                    new_url = f'ESP32_STREAM_URL = "{url}"'
                    
                    updated_content = content.replace(
                        f'ESP32_STREAM_URL = "http://192.168.1.100:81/stream"',
                        f'ESP32_STREAM_URL = "{url}"'
                    )
                    
                    config_file.write_text(updated_content)
                    print(f"\n✓ Updated yolo_inference.py")
                    print(f"  ESP32_STREAM_URL = \"{url}\"")
    else:
