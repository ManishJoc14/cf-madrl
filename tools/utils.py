...
def _resample_metrics_to_length(metrics, target_len):
    """Aggregate metrics lists into a target length using averaged buckets."""
    if target_len <= 0:
        return metrics

    resampled = {}
    for junction_id, series in metrics.items():
        resampled[junction_id] = {}
        for key, values in series.items():
            if not values:
                resampled[junction_id][key] = []
                continue
            if len(values) == target_len:
                resampled[junction_id][key] = values
                continue
            edges = np.linspace(0, len(values), num=target_len + 1, dtype=int)
            aggregated = []
            for idx in range(target_len):
                ...
                aggregated.append(float(np.mean(chunk)))
            resampled[junction_id][key] = aggregated
